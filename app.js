...
const getImages2 = async (url, browser, page) => {
    ...
    try {
        ...
        // const title = await page.$eval('.css-1wdx3tj-DivContainer span ', el => el.textContent) || await page.$eval('.css-199fttw-H1PhotoTitle', el => el.textContent) || 'hi'
        const metaContent = await page.$eval('meta[property="og:description"]', element => element.content);
        const avatar = await page.$eval('a[data-e2e="browse-user-avatar"] div span img', el => el.src);
        const userName = await page.$eval('span[data-e2e="browse-username"]', el => el.textContent); 
        const numberOfImages = imgSrcs.length / 3
        const images = []
        for (let index = 0; index < numberOfImages; index++) {
            images.push(imgSrcs[index])
        ...
        page.close()
        return { images, title: metaContent , userData : {avatar , userName} }
    } catch (error) {
        console.log(error.message)
        throw 'error'
    }
...
const getVideo2 = async (videoUrl, browser, page, callback, errorCallback) => {
    try {
        ...
        const cookieHeader = formatCookiesForAxios(cookies);
        const videoId = Dummy.uniqueId(12)
        download_file(videoSrc, cookieHeader, videoId, () => callback(videoId, images[1], { userName, avatar }))
        page.close()
    } catch (error) {
        console.log('here')
        errorCallback()
...
module.exports = { getUserData, getVideo, getImages, getVideo2, getImages2, getUserInfo }
